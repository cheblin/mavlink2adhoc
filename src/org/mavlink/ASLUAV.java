...
public interface CommonPacks {  @id(91) public static class HIL_CONTROLS{
/**
Timestamp (UNIX Epoch time or time since system boot). The receiving end can infer timestamp format (since
1.1.1970 or since system boot) by checking for the magnitude the number*/
@I long  time_usec;
...
@id(78) public static class COMMAND_INT_STAMPED{
@I  int  utc_time;//UTC time, seconds elapsed since 01.01.1970
@I long  vehicle_timestamp;//Microseconds elapsed since vehicle boot
@I byte  target_system;//System ID
@I byte  target_component;//Component ID
MAV_FRAME frame;//The coordinate system of the COMMAND, as defined by MAV_FRAME enum
MAV_CMD command;//The scheduled action for the mission item, as defined by MAV_CMD enum
@I byte  current;//false:0, true:1
@I byte  autocontinue;//autocontinue to next wp
 float  param1;//PARAM1, see MAV_CMD enum
 float  param2;//PARAM2, see MAV_CMD enum
...
@id(79) public static class COMMAND_LONG_STAMPED{
@I  int  utc_time;//UTC time, seconds elapsed since 01.01.1970
@I long  vehicle_timestamp;//Microseconds elapsed since vehicle boot
@I byte  target_system;//System which should execute the command
@I byte  target_component;//Component which should execute the command, 0 for all components
...
@id(201) public static class SENS_POWER{
...
@id(202) public static class SENS_MPPT{
@I long  mppt_timestamp;//MPPT last timestamp 
 float  mppt1_volt;//MPPT1 voltage 
 float  mppt1_amp;//MPPT1 current 
@I short  mppt1_pwm;//MPPT1 pwm 
@I byte  mppt1_status;//MPPT1 status 
 float  mppt2_volt;//MPPT2 voltage 
 float  mppt2_amp;//MPPT2 current 
@I short  mppt2_pwm;//MPPT2 pwm 
@I byte  mppt2_status;//MPPT2 status 
 float  mppt3_volt;//MPPT3 voltage 
 float  mppt3_amp;//MPPT3 current 
@I short  mppt3_pwm;//MPPT3 pwm 
@I byte  mppt3_status;//MPPT3 status 
}

/**
ASL-fixed-wing controller data*/
@id(203) public static class ASLCTRL_DATA{
@I long  timestamp;//Timestamp
@I byte  aslctrl_mode;//ASLCTRL control-mode (manual, stabilized, auto, etc...)
...
@id(204) public static class ASLCTRL_DEBUG{
@I  int  i32_1;//Debug data
@I byte  i8_1;//Debug data
@I byte  i8_2;//Debug data
...
@id(205) public static class ASLUAV_STATUS{
@I byte  LED_status;//Status of the position-indicator LEDs
@I byte  SATCOM_status;//Status of the IRIDIUM satellite communication system
@I @__( 8 )  byte  Servo_status;//Status vector for up to 8 servos
 float  Motor_rpm;//Motor RPM 
}

/**
Extended EKF state estimates for ASLUAVs*/
@id(206) public static class EKF_EXT{
@I long  timestamp;//Time since system start
...
@id(207) public static class ASL_OBCTRL{
@I long  timestamp;//Time since system start
...
@I byte  obctrl_status;//Off-board computer status
}

/**
Atmospheric sensors (temperature, humidity, ...) */
@id(208) public static class SENS_ATMOS{
@I long  timestamp;//Time since system boot
...
@id(209) public static class SENS_BATMON{
@I long  batmon_timestamp;//Time since system start
 float  temperature;//Battery pack temperature
@I short  voltage;//Battery pack voltage
 short  current;//Battery pack current
@I byte  SoC;//Battery pack state-of-charge
@I short  batterystatus;//Battery monitor status report bits in Hex
@I short  serialnumber;//Battery monitor serial number in Hex
@I  int  safetystatus;//Battery monitor safetystatus report bits in Hex
@I  int  operationstatus;//Battery monitor operation status report bits in Hex
@I short  cellvoltage1;//Battery pack cell 1 voltage
@I short  cellvoltage2;//Battery pack cell 2 voltage
@I short  cellvoltage3;//Battery pack cell 3 voltage
@I short  cellvoltage4;//Battery pack cell 4 voltage
@I short  cellvoltage5;//Battery pack cell 5 voltage
@I short  cellvoltage6;//Battery pack cell 6 voltage
}

/**
Fixed-wing soaring (i.e. thermal seeking) data*/
@id(210) public static class FW_SOARING_DATA{
@I long  timestamp;//Timestamp
@I long  timestampModeChanged;//Timestamp since last mode change
 float  xW;//Thermal core updraft strength
 float  xR;//Thermal radius
 float  xLat;//Thermal center latitude
 float  xLon;//Thermal center longitude
 float  VarW;//Variance W
 float  VarR;//Variance R
 float  VarLat;//Variance Lat
 float  VarLon;//Variance Lon 
 float  LoiterRadius;//Suggested loiter radius
 float  LoiterDirection;//Suggested loiter direction
 float  DistToSoarPoint;//Distance to soar point
 float  vSinkExp;//Expected sink rate at current airspeed, roll and throttle
 float  z1_LocalUpdraftSpeed;//Measurement / updraft speed at current/local airplane position
 float  z2_DeltaRoll;//Measurement / roll angle tracking error
 float  z1_exp;//Expected measurement 1
 float  z2_exp;//Expected measurement 2
 float  ThermalGSNorth;//Thermal drift (from estimator prediction step only)
 float  ThermalGSEast;//Thermal drift (from estimator prediction step only)
 float  TSE_dot;//Total specific energy change (filtered)
 float  DebugVar1;//Debug variable 1
 float  DebugVar2;//Debug variable 2
@I byte  ControlMode;//Control Mode [-]
@I byte  valid;//Data valid [-]
}

/**
Monitoring of sensorpod status*/
@id(211) public static class SENSORPOD_STATUS{
@I long  timestamp;//Timestamp in linuxtime (since 1.1.1970)
@I byte  visensor_rate_1;//Rate of ROS topic 1
@I byte  visensor_rate_2;//Rate of ROS topic 2
@I byte  visensor_rate_3;//Rate of ROS topic 3
@I byte  visensor_rate_4;//Rate of ROS topic 4
@I byte  recording_nodes_count;//Number of recording nodes
@I byte  cpu_temp;//Temperature of sensorpod CPU in
@I short  free_space;//Free space available in recordings directory in [Gb] * 1e2
}

/**
Monitoring of power board status*/
@id(212) public static class SENS_POWER_BOARD{
@I long  timestamp;//Timestamp
@I byte  pwr_brd_status;//Power board status register
@I byte  pwr_brd_led_status;//Power board leds status
...
@id(213) public static class GSM_LINK_STATUS{
@I long  timestamp;//Timestamp (of OBC)
GSM_MODEM_TYPE gsm_modem_type;//GSM modem used
GSM_LINK_TYPE gsm_link_type;//GSM link type
@I byte  rssi;//RSSI as reported by modem (unconverted)
@I byte  rsrp_rscp;//RSRP (LTE) or RSCP (WCDMA) as reported by modem (unconverted)
@I byte  sinr_ecio;//SINR (LTE) or ECIO (WCDMA) as reported by modem (unconverted)
@I byte  rsrq;//RSRQ (LTE only) as reported by modem (unconverted)
}
}}
/**
...
enum GSM_MODEM_TYPE
{ 
GSM_MODEM_TYPE_HUAWEI_E3372;  //HUAWEI LTE USB Stick E3372
 final int
