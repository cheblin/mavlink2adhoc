...
public interface CommonPacks {  @id(115) public static class HIL_STATE_QUATERNION{
/**
Timestamp (UNIX Epoch time or time since system boot). The receiving end can infer timestamp format (since
1.1.1970 or since system boot) by checking for the magnitude the number*/
@I long  time_usec;
...
@I short  stallSpeed;//Aircraft stall speed in cm/s
...
@I  int  utcTime;//UTC time in seconds since GPS epoch (Jan 6, 1980). If unknown set to UINT32_MAX
  ...
  int  gpsAlt;//Altitude (WGS84). UP +ve. If unknown set to INT32_MAX
UAVIONIX_ADSB_OUT_DYNAMIC_GPS_FIX gpsFix;//0-1: no fix, 2: 2D fix, 3: 3D fix, 4: DGPS, 5: RTK
@I byte  numSats;//Number of satellites visible. If unknown set to UINT8_MAX
/**
Barometric pressure altitude (MSL) relative to a standard atmosphere of 1013.2 mBar and NOT bar corrected
altitude (m * 1E-3). (up +ve). If unknown set to INT32_MA*/
  int  baroAltMSL;
@I  int  accuracyHor;//Horizontal accuracy in mm (m * 1E-3). If unknown set to UINT32_MAX
@I short  accuracyVert;//Vertical accuracy in cm. If unknown set to UINT16_MAX
@I short  accuracyVel;//Velocity accuracy in mm/s (m * 1E-3). If unknown set to UINT16_MAX
...
@I short  squawk;//Mode A code (typically 1200 [0x04B0] for VFR)
}
...
ICAROUS_TRACK_BAND_TYPES type2;//See the TRACK_BAND_TYPES enum.
 float  min2;//min angle (degrees)
 float  max2;//max angle (degrees)
ICAROUS_TRACK_BAND_TYPES type3;//See the TRACK_BAND_TYPES enum.
 float  min3;//min angle (degrees)
 float  max3;//max angle (degrees)
ICAROUS_TRACK_BAND_TYPES type4;//See the TRACK_BAND_TYPES enum.
 float  min4;//min angle (degrees)
 float  max4;//max angle (degrees)
ICAROUS_TRACK_BAND_TYPES type5;//See the TRACK_BAND_TYPES enum.
...
@I short  adc4;//ADC output 4.
@I short  adc5;//ADC output 5.
@I short  adc6;//ADC output 6.
}
...
@I short  shutter_speed;//Divisor number //e.g. 1000 means 1/1000 (0 means ignore).
@I byte  aperture;//F stop number x 10 //e.g. 28 means 2.8 (0 means ignore).
@I byte  iso;//ISO enumeration from 1 to N //e.g. 80, 100, 200, Etc (0 means ignore).
@I byte  exposure_type;//Exposure type enumeration from 1 to N (0 means ignore).
/**
Command Identity (incremental loop: 0 to 255). //A command sent multiple times will be executed or pooled
just once*/
@I byte  command_id;
@I byte  engine_cut_off;//Main engine cut-off time before camera trigger (0 means no cut-off).
@I byte  extra_param;//Extra parameters enumeration (0 means ignore).
...
@I byte  zoom_pos;//1 to N //Zoom's absolute position (0 means ignore).
 byte  zoom_step;//-100 to 100 //Zooming step value to offset zoom from the current position.
@I byte  focus_lock;//0: unlock focus or keep unlocked, 1: lock focus or keep locked, 3: re-lock focus.
@I byte  shot;//0: ignore, 1: shot or start filming.
/**
Command Identity (incremental loop: 0 to 255)//A command sent multiple times will be executed or pooled
just once*/
@I byte  command_id;
...
@I byte  stab_roll;//(1 = yes, 0 = no).
@I byte  stab_pitch;//(1 = yes, 0 = no).
@I byte  stab_yaw;//(1 = yes, 0 = no).
}
...
@I byte  target_component;//Component ID.
  ...
  int  input_b;//Roll (centi-degrees) or lon (degE7) depending on mount mode.
  int  input_c;//Yaw (centi-degrees) or alt (cm) depending on mount mode.
@I byte  save_position;//If "1" it will save current trimmed position on EEPROM (just valid for NEUTRAL and LANDING).
}
...
@I byte  count;//Total number of points (for sanity checking).
...
@I byte  noise;//Background noise level.
@I byte  remnoise;//Remote background noise level.
@I short  rxerrors;//Receive errors.
@I short  fixed;//Count of error corrected packets.
}
...
@I  int  last_recovery;//Time (since boot) of last successful recovery.
@I  int  last_clear;//Time (since boot) of last all-clear.
@I short  breach_count;//Number of fence breaches.
LIMIT_MODULE mods_enabled;//AP_Limit_Module bitfield of enabled modules.
LIMIT_MODULE mods_required;//AP_Limit_Module bitfield of required modules.
LIMIT_MODULE mods_triggered;//AP_Limit_Module bitfield of triggered modules.
}
...
@I byte  count;//Total number of points (for sanity checking).
  int  lat;//Latitude of point.
  int  lng;//Longitude of point.
 short  alt;//Transit / loiter altitude relative to home.
 short  break_alt;//Break altitude relative to home.
@I short  land_dir;//Heading to aim for when landing.
RALLY_FLAGS flags;//Configuration flags.
}
...
@I short  img_idx;//Image index.
CAMERA_STATUS_TYPES event_id;//Event type.
...
@I short  img_idx;//Image index.
 ...
 float  alt_rel;//Altitude (Relative to HOME location).
...
CAMERA_FEEDBACK_FLAGS flags;//Feedback flags.
@I_ short  completed_captures;//Completed image captures.
}
...
@I @__( 200 )  byte  data;//Log data block.
}
...
@I byte  pattern;//Pattern (see LED_PATTERN_ENUM).
@I byte  custom_len;//Custom Byte Length.
@I @__( 24 )  byte  custom_bytes;//Custom Bytes.
}
...
@I byte  attempt;//Attempt number.
@I byte  completion_pct;//Completion percentage.
@I @__( 10 )  byte  completion_mask;//Bitmask of sphere sections (see http://en.wikipedia.org/wiki/Geodesic_grid).
...
@I byte  cal_mask;//Bitmask of compasses being calibrated.
MAG_CAL_STATUS cal_status;//Calibration Status.
@I byte  autosaved;//0=requires a MAV_CMD_DO_ACCEPT_MAG_CAL, 1=saved to parameters.
...
@I_()  float  orientation_confidence;//Confidence in orientation (higher is better).
MAV_SENSOR_ORIENTATION old_orientation;//orientation before calibration.
MAV_SENSOR_ORIENTATION new_orientation;//orientation after calibration.
@I_()  float  scale_factor;//field radius correction factor
}
...
@I_()  float  airspeed_variance;//Airspeed variance.
}
...
@id(195) public static class DEEPSTALL{
  ...
  int  path_lon;//Final heading start point, longitude.
  int  arc_entry_lat;//Arc entry point, latitude.
  int  arc_entry_lon;//Arc entry point, longitude.
...
DEEPSTALL_STAGE stage;//Deepstall stage.
}
...
@I byte  health;//EFI Health status
 ...
 float  rpm;//RPM
 float  fuel_consumed;//Fuel Consumed (grams)
 float  fuel_flow;//Fuel Flow Rate (g/min)
 float  engine_load;//Engine Load (%)
 float  throttle_position;//Throttle Position (%)
 float  spark_dwell_time;//Spark Dwell Time (ms)
 float  barometric_pressure;//Barometric Pressure (kPa)
 float  intake_manifold_pressure;//Intake Manifold Pressure (kPa)(
 float  intake_manifold_temperature;//Intake Manifold Temperature (degC)
 float  cylinder_head_temperature;//cylinder_head_temperature (degC)
 float  ignition_timing;//Ignition timing for cylinder i (Crank Angle degrees)
 float  injection_time;//Injection time for injector i (ms)
}
...
@I byte  bus;//Bus number.
@I byte  address;//Bus address.
@__(40) String  busname;//Name of device on bus (for SPI).
@I byte  regstart;//First register to read.
@I byte  count;//Count of registers to read.
}
...
@I byte  count;//Count of bytes read.
@I @__( 128 )  byte  data;//Reply data.
}
...
@I byte  address;//Bus address.
@__(40) String  busname;//Name of device on bus (for SPI).
@I byte  regstart;//First register to write.
@I byte  count;//Count of registers to write.
@I @__( 128 )  byte  data;//Write data.
}
...
@I long  time_delta_usec;//Time since the last reported camera frame.
 ...
 @__( 3 )  float  position_delta;//Change in position from previous to current frame rotated into body frame (0=forward, 1=right, 2=down)
...
@I @__( 4 )  short  totalcurrent;//Total current.
@I @__( 4 )  short  rpm;//RPM (eRPM).
@I @__( 4 )  short  count;//count of telemetry packets received (wraps at 65535).
}
...
State flags for ADS-B transponder dynamic report*/
 @Flags enum UAVIONIX_ADSB_OUT_DYNAMIC_STATE
{ 
;
...
Transceiver RF control flags for ADS-B transponder dynamic reports*/
 @Flags enum UAVIONIX_ADSB_OUT_RF_SELECT
{ 
;
...
Status flags for ADS-B transponder dynamic output*/
 @Flags enum UAVIONIX_ADSB_RF_HEALTH
{ 
;
...
UAVIONIX_ADSB_OUT_RESERVED = 7;  
 ...
 @Flags enum ICAROUS_TRACK_BAND_TYPES
{ 
;
...
GIMBAL_AXIS_ROLL = 2;  //Gimbal roll axis.
 ...
 @Flags enum GIMBAL_AXIS_CALIBRATION_STATUS
{ 
;
...
GIMBAL_AXIS_CALIBRATION_STATUS_FAILED = 2;  //Axis calibration failed.
 ...
 @Flags enum GIMBAL_AXIS_CALIBRATION_REQUIRED
{ 
;
...
GOPRO_FRAME_RATE_12_5 = 13;  //12.5 FPS.
 ...
 @Flags enum GOPRO_FIELD_OF_VIEW
{ 
;
...
GOPRO_PROTUNE_GAIN_6400 = 4;  //ISO 6400.
 ...
 @Flags enum GOPRO_PROTUNE_SHARPNESS
{ 
;
