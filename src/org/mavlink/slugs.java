...
public interface CommonPacks {  @id(100) public static class OPTICAL_FLOW{
/**
Timestamp (UNIX Epoch time or time since system boot). The receiving end can infer timestamp format (since
1.1.1970 or since system boot) by checking for the magnitude the number*/
@I long  time_usec;
...
@id(170) public static class CPU_LOAD{
@I byte  sensLoad;//Sensor DSC Load
@I byte  ctrlLoad;//Control DSC Load
@I short  batVolt;//Battery Voltage
}

/**
Accelerometer and gyro biases.*/
@id(172) public static class SENSOR_BIAS{
...
@id(173) public static class DIAGNOSTIC{
 float  diagFl1;//Diagnostic float 1
 float  diagFl2;//Diagnostic float 2
 float  diagFl3;//Diagnostic float 3
 short  diagSh1;//Diagnostic short 1
 short  diagSh2;//Diagnostic short 2
 short  diagSh3;//Diagnostic short 3
}

/**
Data used in the navigation algorithm.*/
@id(176) public static class SLUGS_NAVIGATION{
...
@I byte  fromWP;//Origin WP
@I byte  toWP;//Destination WP
@I short  h_c;//Commanded altitude (MSL)
}

/**
Configurable data log probes to be used inside Simulink*/
@id(177) public static class DATA_LOG{
...
@id(179) public static class GPS_DATE_TIME{
@I byte  year;//Year reported by Gps 
@I byte  month;//Month reported by Gps 
@I byte  day;//Day reported by Gps 
@I byte  hour;//Hour reported by Gps 
@I byte  min;//Min reported by Gps 
@I byte  sec;//Sec reported by Gps  
@I byte  clockStat;//Clock Status. See table 47 page 211 OEMStar Manual  
@I byte  visSat;//Visible satellites reported by Gps  
@I byte  useSat;//Used satellites in Solution  
@I byte  GppGl;//GPS+GLONASS satellites in Solution  
@I byte  sigUsedMask;//GPS and GLONASS usage mask (bit 0 GPS_used? bit_4 GLONASS_used?)
@I byte  percentUsed;//Percent used GPS
}

/**
Mid Level commands sent from the GS to the autopilot. These are only sent when being operated in mid-level
commands mode from the ground*/
@id(180) public static class MID_LVL_CMDS{
@I byte  target;//The system setting the commands
 float  hCommand;//Commanded altitude (MSL)
...
@id(181) public static class CTRL_SRFC_PT{
@I byte  target;//The system setting the commands
CONTROL_SURFACE_FLAG bitfieldPt;//Bitfield containing the passthrough configuration, see CONTROL_SURFACE_FLAG ENUM.
}

/**
Orders generated to the SLUGS camera mount. */
@id(184) public static class SLUGS_CAMERA_ORDER{
@I byte  target;//The system reporting the action
 byte  pan;//Order the mount to pan: -1 left, 0 No pan motion, +1 right
 byte  tilt;//Order the mount to tilt: -1 down, 0 No tilt motion, +1 up
 byte  zoom;//Order the zoom values 0 to 10
/**
...
@id(185) public static class CONTROL_SURFACE{
@I byte  target;//The system setting the commands
@I byte  idSurface;//ID control surface send 0: throttle 1: aileron 2: elevator 3: rudder
 float  mControl;//Pending
 float  bControl;//Order to origin
}

/**
Transmits the last known position of the mobile GS to the UAV. Very relevant when Track Mobile is enable*/
@id(186) public static class SLUGS_MOBILE_LOCATION{
@I byte  target;//The system reporting the action
...
/**
Control for camara.*/
@id(188) public static class SLUGS_CONFIGURATION_CAMERA{
@I byte  target;//The system setting the commands
@I byte  idOrder;//ID 0: brightness 1: aperture 2: iris 3: ICR 4: backlight
@I byte  order;//1: up/on 2: down/off 3: auto/reset/no action
}

/**
Transmits the position of watch*/
@id(189) public static class ISR_LOCATION{
@I byte  target;//The system reporting the action
 float  latitude;//ISR Latitude
 float  longitude;//ISR Longitude
 float  height;//ISR Height
@I byte  option1;//Option 1
@I byte  option2;//Option 2
@I byte  option3;//Option 3
}

/**
Transmits the readings from the voltage and current sensors*/
@id(191) public static class VOLT_SENSOR{
@I byte  r2Type;//It is the value of reading 2: 0 - Current, 1 - Foreward Sonar, 2 - Back Sonar, 3 - RPM
@I short  voltage;//Voltage in uS of PWM. 0 uS = 0V, 20 uS = 21.5V 
/**
...
@id(192) public static class PTZ_STATUS{
@I byte  zoom;//The actual Zoom Value
 short  pan;//The Pan value in 10ths of degree
 short  tilt;//The Tilt value in 10ths of degree
}

/**
Transmits the actual status values UAV in flight*/
@id(193) public static class UAV_STATUS{
@I byte  target;//The ID system reporting the action
...
@id(194) public static class STATUS_GPS{
@I short  csFails;//Number of times checksum has failed
/**
...
@I byte  gpsQuality;
@I byte  msgsType;//Indicates if GN, GL or GP messages are being received
@I byte  posStatus;//A = data valid, V = data invalid
...
@id(195) public static class NOVATEL_DIAG{
@I byte  timeStatus;//The Time Status. See Table 8 page 27 Novatel OEMStar Manual
@I  int  receiverStatus;//Status Bitfield. See table 69 page 350 Novatel OEMstar Manual
@I byte  solStatus;//solution Status. See table 44 page 197
@I byte  posType;//position type. See table 43 page 196
@I byte  velType;//velocity type. See table 43 page 196
 float  posSolAge;//Age of the position solution
@I short  csFails;//Times the CRC has failed since boot
}

/**
Diagnostic data Sensor MCU*/
@id(196) public static class SENSOR_DIAG{
 float  float1;//Float field 1
 float  float2;//Float field 2
 short  int1;//Int 16 field 1
 byte  char1;//Int 8 field 1
}

/**
The boot message indicates that a system is starting. The onboard software version allows to keep track
of onboard soft/firmware revisions. This message allows the sensor and control MCUs to communicate version
numbers on startup*/
@id(197) public static class BOOT{
@I  int  version;//The onboard software version
}
}}
/**
...
has control of the surface, and if not then the autopilot has control of the surface.*/
 @Flags enum CONTROL_SURFACE_FLAG
{ 
;
